...
(define (script-fu-export-no-crater-selected-regions image drawable)
  ;; Start
  ...
  (when (= 0 (car (gimp-selection-is-empty image)))
    (let ((number 1) (prefix "") (suffix ""))
      ...
      (let* ((parts (strbreakup (car (gimp-image-get-filename image)) "."))
        ...
        (set! prefix (string-append coextension "_nocrater_selection-" ))
        (set! suffix (string-append ".jpg")))
...
(script-fu-register "script-fu-export-no-crater-selected-regions"
                    "Export No Crater Selected Regions"
...
(script-fu-menu-register "script-fu-export-no-crater-selected-regions" "<Image>/Select")
